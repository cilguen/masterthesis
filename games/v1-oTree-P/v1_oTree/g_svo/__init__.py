...
def group_rows():
    ...
    d = defaultdict(list)
    for row in read_csv(__name__ + '/g_SVO.csv', Choice):
...
class Player(BasePlayer):
    g_svo_choice = models.IntegerField(choices=list(range(9)))
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        ...
        for p in player.in_all_rounds():
            row = C.ROWS[p.round_number][p.g_svo_choice]
